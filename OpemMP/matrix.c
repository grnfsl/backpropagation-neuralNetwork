...
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(A, A_T) private(i, j)
    for(i = 0; i < A->n; ++i)
        for(j = 0; j < A_T->n; ++j)
...
Mat* mat_mat_mult(Mat* A, Mat* B, int thread_count)
{
    ...
    unsigned i, j, k;
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(A, B, C) private(i, j, k)
    for(i = 0; i < A->n; ++i)
        for(j = 0; j < B->m; ++j){
            double tmp = 0.0;
            for(k = 0; k < B->n; ++k)
...
//    double* y = (double*)malloc(A->n * sizeof (double));
    ...
    unsigned i, j;
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(A, y, x, x_size) private(i, j)
    for(i = 0; i < A->n; ++i){
        double tmp = 0.0;
        for(j = 0; j < x_size; ++j)
...
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(size, A, B, C)
    for(i = 0; i < size; ++i)
...
void vect_vect_sub(double* x, double* y, unsigned int size, double* vect_c, int thread_count)
{
    unsigned int i;
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(size, x, y, vect_c)
    for(i = 0; i < size; ++i)
        vect_c[i] = x[i] - y[i];
}
...
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(vect, vect_c, size, scalar)
    for(i = 0; i < size; ++i)
...
double* vect_mult_ew(double* x, double* y, unsigned int size, int thread_count) //elementwise
{
    ...
    unsigned int i;
#pragma omp parallel for num_threads(thread_count) \
    default(none) shared(vect_c, x, y, size)
    for(i = 0; i < size; ++i)
        vect_c[i] = x[i] * y[i];
